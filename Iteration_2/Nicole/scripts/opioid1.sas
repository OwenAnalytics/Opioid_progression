** Investigate Opioid Data - MarketScan Medicaid                  **;
** 04.17.18 N. Ridenour                                           **;
** from trvlapp0852: /government/opioid/scripts/opioid_final2.sas **;
** trvlapp3152 /opioid/scripts/opioid.sas                         **;

libname mdcd    '/data/mscan';
libname op      '/home/u0112740/opioid/data';
libname library '/home/u0112740/opioid/library';
...
data op.elig13_16_1;
  merge mdcd.medicaid_a_2013_v4 (keep = client enrolid enrmon rename = (enrmon = enrmon13))
		mdcd.medicaid_a_2014_v2 (keep = client enrolid enrmon age rename = (enrmon = enrmon14))
		mdcd.medicaid_a_2015_v2 (keep = client enrolid enrmon rename = (enrmon = enrmon15))
		mdcd.medicaid_a_2016_v1 (keep = client enrolid enrmon recipzip sex stdrace msa recipcty recipgeoloc rural 
...
%macro elig(yr,ver);
  proc sql;
    ...
    select b.* 
    from op.elig13_16_1 as a inner join mdcd.medicaid_a_20&yr._&ver as b
    on a.client = b.client & a.enrolid = b.enrolid;
  quit;
%mend elig;
* adding first 6 months of 2017 *; *%elig(17,v0);
...
%macro opioid(yr,ver);
  ...
  ** Limit to members in Elig dataset **;
  proc sql;
    ...
    select b.* 
    from op.elig13_16_1 as a inner join mdcd.medicaid_d_20&yr._&ver as b
    on a.client = b.client & a.enrolid = b.enrolid;
  quit;
  data op.rx&yr;
  ...
  proc freq data=op.rx&yr;
	...
	tables opioid naloxone buprenorphine methadone short_acting long_acting fentanyl benzodiazepine naltrexone antidepressant 
      antipsychotic op_lax nat_opioid syn_opioid semi_opioid muscle deaclas 
      opioid * gt7d 
  ...
  * Assumes sorted by svcdate, so age will be age at last Rx *;
  data op.rx&yr._1 (keep = client enrolid age sex opioid1 naloxone1 buprenorphine1 methadone1 short_acting1 long_acting1
      ...
      op_benz op_dep op_psy op_lax opioid_days muscle1 good_pot1 bad_pot1 op_lax_c  barbiturate1 muscle1 trinity);
	...
	* set flags by member *;
	retain opioid1 naloxone1 buprenorphine1 methadone1 short_acting1 long_acting1 fentanyl1 benzodiazepine1 naltrexone1 
      ...
      bad_pot1 good_pot1 barbiturate1 muscle1 trinity 0;
...
proc freq data=op.rx&yr._1;
    tables nat_opioid1  * semi_opioid1 * syn_opioid1
      ...
      op_lax_c good_pot1 bad_pot1 barbiturate1 muscle1 trinity / missprint; 
  run;
  proc freq data=op.rx&yr._1; 
    where opioid1 > 0; 
  ...
  ** Medical **;
  * How many members? *;
  proc sort data=mdcd.medicaid_s_20&yr._&ver out = ip&yr (keep = client enrolid) nodupkey; by client enrolid; run;
  proc sort data=mdcd.medicaid_o_20&yr._&ver out = op&yr (keep = client enrolid) nodupkey; by client enrolid; run;
  ...
  * OP *;
  proc sql;
    ...
    select b.* 
    from op.elig13_16_1 as a inner join mdcd.medicaid_o_20&yr._&ver as b
    on a.client = b.client & a.enrolid = b.enrolid;
  quit;
  * IP *;
  proc sql;
    ...
    select b.* 
    from op.elig13_16_1 as a inner join mdcd.medicaid_s_20&yr._&ver as b
    on a.client = b.client & a.enrolid = b.enrolid;
  quit;
  data op.med&yr (drop = i);
	...
	* Hospice_Morphine *;
	if proctyp in ('1','7') then do;
	  ...
	  else if proc1 in &morph          then morphine = 1;
  ...
  proc freq data=op.med&yr;
    title "Med &yr claims";
	tables mh_dxcat cancer_dxcat dxcat mdc sud oud oudr adv od morphine treat treat_rx nas mh cancer hospice death mal_neo oth_neo 
...
data op.med&yr;
  set op.med&yr;
  ** ED flags **;
  if put(proc1,$ED_CPT.)     = 'ED_CPT'   then ed_cpt = 1; else ed_cpt = 0;
  if put(revcode,$ED_UBREV.) = 'ED_UBREV' then ed_rev = 1; else ed_rev = 0;
  if ed_cpt or ed_rev then ed = 1; else ed = 0;
  if stdprov in (200, 202, 204, 206, 240, 245, 400, 410, 825, 845) then pcp = 1; else pcp = 0;
run;
proc freq data=op.med&yr; tables ed_cpt ed_rev ed pcp / missprint; 
  * Summarize by member *;
  * Sorted by svcdate, so age will be age at last claim *;
  proc sort data=op.med&yr; by client enrolid svcdate; run;
  data op.med&yr._1 (keep = client enrolid age sex sud1 oud1 oudr1 adv1 od1 hospice1 morphine1 treat1 treat_rx1 death1 
      cancer1 mh1 nas1 nas_mom1 cancer_dxcat1 mh_dxcat1 dxcat_psy11 dxcat_tra82 mdc20 mdc21 mdc19);
    set op.med&yr (keep = client enrolid age sex sud oud oudr adv od hospice morphine treat treat_rx death cancer mh nas 
      nas_mom cancer_dxcat mh_dxcat mdc dxcat);
	by client enrolid;
	* set flags by member *;
	retain sud1 oud1 oudr1 adv1 od1 morphine1 treat1 treat_rx1 hospice1 death1 cancer1 mh1 nas1 nas_mom1 cancer_dxcat1 mh_dxcat1 dxcat_psy11 dxcat_tra82
      mdc20 mdc19 mdc21 0;
	if first.enrolid then do;
	  sud1      = 0;
      oud1      = 0;
 ...
 proc freq data=op.med&yr._1;
    title "med person";
    tables age sex sud1 oud1 oudr1 adv1 od1 hospice1 morphine1 treat1 treat_rx1 death1 cancer1 mh1 nas1 nas_mom1
	       oud1  * cancer1
		   oud1  * mh1
           oud1  * sud1 
		   oud1  * oudr1 
  ...
  * Admissions - only place to get ub_surg px *;
  proc sort data=mdcd.medicaid_i_20&yr._&ver out = adm&yr (keep = client enrolid) nodupkey; by client enrolid; run;
  proc sql;
    create table op.adm&yr as
    select b.* 
    from op.elig13_16_1 as a inner join mdcd.medicaid_i_20&yr._&ver as b
    on a.client = b.client & a.enrolid = b.enrolid;
  quit;
...
/** check OUD via dx and dxcat **;
data op.oud_mismatch;
  set op.med14 (where = (oud = 1 and dxcat ne 'PSY11'));
run;
proc print data=op.oud_mismatch (obs = 5); title 'OUD mismatch'; where dxcat = 'PSY04'; var client enrolid svcdate dx1 dx2 dx3 dx4 dxcat; run;

proc freq data=op.oud_mismatch; tables dxcat / missprint; run;
*/
%macro new (by,py);
...
data morphine_px;
  set op.med14 (keep = client enrolid svcdate morphine rename = (svcdate = px_svcdate) where = (morphine = 1));
run;
data morphine_rx;
...
proc sql;
  ...
  on a.client = b.client & a.enrolid = b.enrolid;
quit;
data op.morphine (drop = morphine);
  set op.morphine;
...
%macro bree_yr(q1,q2,q3,q4,yr);
  data chronic_&yr (keep = client enrolid n_chronic n_oud);
  ...
  run;
  proc freq data=chronic_&yr; title "Chronic # Qs 20&yr"; tables n_chronic * n_oud / missprint; run;
%mend bree_yr;
...
data op.chronic16;
  ...
  if nin & bin;
run;
proc print data=op.chronic16 (obs = 5); run;
proc freq data=op.chronic16; tables oud1 od1 death1 / missprint; run;
